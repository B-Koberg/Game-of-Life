...
boundary conditions. A random initial state is used for the benchmark runs, 
with about half of the cells alive at the start.
...
complete however, as cells near a partition boundary require neighbour values
...
by \texttt{src/hdf5\_utils.f90}. For the MPI version there is an additional 
\texttt{src/mpi\_utils.f90} file for MPI tools like halo exchange.

Two arrays are used are \texttt{board\_current} and \texttt{board\_next} which 
contain $u^t$ and $u^{t+1}$. Every new cell reads only the old state. For a 
grid with $n_x n_y$ cells and $F$ generations, the update work is $\mathcal{O}(F n_x n_y)$ 
and the storage requirement is $\mathcal{O}(n_x n_y)$ for the two boards.
...
100, 500, 1000, 1250, and 1500 while the number of threads or MPI ranks is 1, 2, 3, 4, and 8.
...
each run, differences in the output cannot be attributed directly to the
...
reference, because one OpenMP thread and several OpenMP threads must produce identical
...
The MPI curves show the same pattern, with the best MPI speedup at about 1.20 at
...
overviews of the available data. The OpenMP and MPI results are shown side by 
side in Figs.~\ref{fig:openmp-weak} and~\ref{fig:mpi-weak}.
