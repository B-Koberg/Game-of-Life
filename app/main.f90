program GOL
    ...
    use hdf5
    use parameters
    use mpi_utils, only: local_rows, exchange_halos, gather_and_save
    ...
    !Board auf die einzelnen Prozesse aufteilen
    local_ny = local_rows(rank, size) 

    allocate(board_current(nx, 0:local_ny+1))
    allocate(board_next(nx, 0:local_ny+1))
    call initialize_board(board_current, local_ny)
    board_next = 0
    ...
    saved_frame = 1
    call gather_and_save(board_current, local_ny, rank, size, saved_frame, dset_id, filespace_id, memspace_id)
    ...
    do frame = 2, frames + 1
        ...
        if (rank == 0 .and. mod(int(perc), 10) == 0) then
            call print_time(rank, "Fortschritt: "//trim(itoc(int(perc)))//"%")
        end if
    end do
    ...
    deallocate(board_current, board_next)
    call MPI_Finalize()
